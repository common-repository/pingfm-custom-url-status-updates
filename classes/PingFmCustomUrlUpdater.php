...
class PingFmCustomUrlUpdater implements PingFmCustomUrlConstants {
     ...
     */
    public function do_upgrade_check() {
        $options = PingFmCustomUrlOptions::factory();
        $options = $this->_sync_options_object($options);
     ...
     * @return        bar
     */
    private function _sync_options_object($options) {
        $new_options = PingFmCustomUrlOptions::factory(true);
