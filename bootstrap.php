...
/*
Plugin Name: Ping.fm Custom URL
...
// First things first: let's make sure we're running a compatible version of
// PHP before we go any further

if (version_compare(PHP_VERSION, '5.2.0', '>=')) {
    // For accessing filesystem-based resources (includes, activation hooks, etc.)
    define('PCUSU_PLUGIN_FILE', __FILE__);
    define('PCUSU_PLUGIN_DIR', dirname(__FILE__) . '/');

    // For accessing web-based resources (images, style sheets, etc.)
    define('PCUSU_HTTP_PATH', WP_PLUGIN_URL . '/' . str_replace(basename(__FILE__), "", plugin_basename(__FILE__)));

    // Debugging settings (should be disabled under normal circumstances)
    define('PCUSU_DEBUG_FILE', PCUSU_PLUGIN_DIR . 'debug.log');
    define('PCUSU_DEBUG', true);

    // Bring in the files we need everywhere...
    ...
    // This doesn't do any actual work. It just hooks our stuff into WordPress.
